...
/**
 * RC (Radiance Cascades) class extends GradientField
 * This class implements a radiance cascade rendering system for real-time global illumination
 */
class RC extends GradientField {
  
  /**
   * Initializes the RC renderer with necessary parameters and UI controls
   * Sets up GPU timers, sliders, and WebGL uniforms
   */
  innerInitialize() {
  ...
  /**
   * Initializes or updates key rendering parameters
   * Calculates radiance cascades based on screen dimensions
   * @param {boolean} setUniforms - Whether to update WebGL uniforms with new values
   */
  ...
  /**
   * Handles the overlay rendering pass
   * Manages frame buffers and mobile/desktop rendering differences
   * @param {WebGLTexture} inputTexture - Input texture for overlay
   * @param {boolean} preRc - Pre-radiance cascade flag
   */
  ...
  /**
   * Triggers a draw operation
   * Handles overlay mode separately from normal drawing
   */
  triggerDraw() {
    if (this.overlay) {
      this.renderer.setRenderTarget(null);
  ...
  /**
   * Defines canvas interaction modifications
   * Sets up event handlers for drawing, mouse movement, and sun toggle
  ...
  /**
   * Performs the radiance cascade rendering pass
   * Manages multiple cascade levels and GPU timing
   * @param {WebGLTexture} gradientFieldTexture - Gradient field input
   * @param {WebGLTexture} distanceFieldTexture - Distance field input
   * @param {WebGLTexture} drawPassTexture - Draw pass input
   * @returns {WebGLTexture} Resulting RC texture
   */
   ...
   * Orchestrates the entire rendering pipeline including seed, JFA, distance field, and gradient passes
   */
  doRenderPass() {
    if (this.frame == 0) {
      if (this.stage == 0) {
   ...
   * Updates GPU timer and manages frame counter
   */
  finishRenderPass() {
  ...
  /**
   * Initiates a new render pass
   * Triggers animation frame if not already animating
   */
  renderPass() {
   ...
   * Continues animation while recent user interaction exists
   */
  animate() {
    this.animating = true;
  ...
  /**
   * Clears all render targets and resets state
   */
  clear() {
    this.lastFrame = null;
    if (this.initialized) {
  ...
  /**
   * Initializes event listeners and UI controls
   * Sets up checkbox handlers and slider interactions
   */
  load() {
...
// Export the RC class as the default export
export default RC;
